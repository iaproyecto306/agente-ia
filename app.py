...
traducciones = {
    ...
    # Otros idiomas... (mantenidos igual)
}

# --- 3. ESTILOS CSS (Aura Estabilizada) ---
st.markdown("""
<style>
    ...
    div.stButton > button[kind="primary"] { 
        ...
        box-shadow: 0 0 20px rgba(0, 210, 255, 0.4) !important; transition: all 0.4s !important; 
    ...
    /* PLANES */
    .card-wrapper { transition: all 0.6s cubic-bezier(0.165, 0.84, 0.44, 1); border-radius: 12px; height: 480px; }
    ...
    .pro-card { border: 1px solid rgba(0, 210, 255, 0.3) !important; box-shadow: 0 0 25px rgba(0, 210, 255, 0.1); }
    .agency-card { border: 1px solid rgba(221, 160, 221, 0.3) !important; box-shadow: 0 0 25px rgba(221, 160, 221, 0.1); }
    ...
    .video-placeholder {
        ...
        animation: auraPulse 4s ease-in-out infinite, adCarousel 18s infinite;
        box-shadow: 0 0 20px rgba(0, 210, 255, 0.2);
    }
    
    @keyframes auraPulse {
        0% { box-shadow: 0 0 15px rgba(0, 210, 255, 0.2); border-color: rgba(0, 210, 255, 0.3); }
        50% { box-shadow: 0 0 35px rgba(0, 210, 255, 0.4); border-color: rgba(0, 210, 255, 0.6); }
        100% { box-shadow: 0 0 15px rgba(0, 210, 255, 0.2); border-color: rgba(0, 210, 255, 0.3); }
    }
...
with c2:
    ...
    <div class="video-placeholder">
        <div style="position: absolute; top: 15px; left: 15px; background: #00d2ff; color: black; padding: 5px 14px; border-radius: 4px; font-size: 0.75rem; font-weight: 900; box-shadow: 0 0 10px rgba(0,0,0,0.3);">{L["p_destacada"]}</div>
        <div style="background: linear-gradient(0deg, rgba(0,0,0,0.8) 0%, transparent 100%); width: 100%; padding: 20px; text-align: center;">{L["comunidad"]}</div>
    </div>
    ''', unsafe_allow_html=True)
...
with col2:
    ...
    st.markdown(f"<div class='card-wrapper pro-card'><div class='glass-container'><div style='position: absolute; top: -12px; left: 50%; transform: translateX(-50%); background-color: #00d2ff; color: black; padding: 6px 18px; border-radius: 20px; font-weight: 900; font-size: 0.85rem; z-index: 10;'>{L['popular']}</div><h3 style='color:#00d2ff;'>{L['plan2']}</h3><h1>$49</h1><hr style='border-color:#00d2ff;opacity:0.3;'>{desc_p}</div></div>", unsafe_allow_html=True)
